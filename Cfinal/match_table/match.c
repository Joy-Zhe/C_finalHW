...
typedef struct s_Token{//链表表头存放当前串的信息（因为无需记录第一个出现的子串），而每次遍历都是从头开始的，故可将表头作为索引
    int index;//存放找到的
	char contain[10001];// 仅在存放表头时有效，即s_Token[i]->contain
    struct s_Token *next;
}LinkList;
LinkList *M[100001];
int M_book = 0;

LinkList *map_create(/*int n, */char *subToken, int n);//遇到未加入映射表的subToken，建立新的键, n为第一次遇到时的Token编号，后续编号从M[]->next开始
...
int /*mapping_*/main (){//之后改成映射函数
	int cnt_token = 2, i = 1;
	//Mtoken[1].ch = "abababab";
    //Mtoken[2].ch = "dnabcba";
	char subToken[100001];//存放当次子串
    //char Token[100001][100001];//母串 //改为使用新的结构体数组
    int index_Token = 1, sublen = 2, pos = 0, index_subToken = 1;
    //取出键
	for(i = 1; i <= cnt_token; i++){
		pos = 0;
		for(int j = pos; j <= strlen(Mtoken[i].ch) - sublen; j++){
...
LinkList *map_create(/*int n, */char *subToken, int n){
	LinkList *head;//定义头节点
	int i = 0;
